...
async function submitWork() {
  ...
  for (const item of integrantes.value) {
    if (item.ra.trim() && !item.nome) {
      errorMsg.value = `RA "${item.ra}" não identificado. Verifique antes de enviar.`; return
    }
  }
  ...
  try {
    ...
    if (tipoSubmissao.value === 'arquivo') {
      ...
      if (orientadorNome.value.trim())   form.append('orientadorNome', orientadorNome.value.trim())
      if (orientadorEmail.value.trim())  form.append('orientadorEmail', orientadorEmail.value.trim())
  ...
  <div class="sub-view">
    ...
    <section class="card mb">
      ...
      <div v-for="(item, idx) in integrantes" :key="idx" class="integrante-row">
        ...
        <button v-if="integrantes.length > 1" class="btn-remove" title="Remover" :disabled="submitting" @click="removeIntegrante(idx)">×</button>
      </div>
      <button class="btn btn--secondary btn--sm" :disabled="submitting" @click="addIntegrante">+ Adicionar integrante</button>
    </section>
    ...
    <section v-if="hasWork && myWork" class="card mb">
      ...
      <p v-if="myWork.status === 'Reprovado'" class="resubmit-hint">Seu trabalho foi reprovado. Corrija e reenvie abaixo.</p>
    </section>
    ...
    <template v-if="!hasWork || myWork?.status === 'Reprovado'">
      ...
      <section class="card mb">
        ...
        <template v-if="tipoSubmissao === 'manual'">
          <div class="manual-divider">Informações do orientador</div>

          <div class="form-grid">
            <div class="form-group">
              <label>Nome completo do orientador (com titulação)</label>
              <input v-model="orientadorNome" type="text" class="input-field" placeholder="Ex.: Prof. Me. Eng. João Silva" :disabled="submitting" />
            </div>
            <div class="form-group">
              <label>E-mail do orientador</label>
              <input v-model="orientadorEmail" type="email" class="input-field" placeholder="Ex.: joao.silva@prof.fae.br" :disabled="submitting" />
            </div>
          </div>
...
/* Manual fields */
.manual-divider { font-size: .78rem; font-weight: 700; text-transform: uppercase; letter-spacing: .06em; color: #0d631b; margin: 1.1rem 0 .6rem; padding-bottom: .3rem; border-bottom: 1px solid #d1fae5; }
