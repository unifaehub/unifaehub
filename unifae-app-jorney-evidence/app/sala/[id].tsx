  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <Text style={styles.date}>
        Data: {new Date(room.dataEvento + 'T00:00:00').toLocaleDateString('pt-BR')}
      </Text>
      ...
      <View style={styles.section}>
        ...
        {banca.length === 0 ? (
          <Text style={styles.bancaItem}>Banca não carregada.</Text>
        ) : (
          banca.map((rp) => (
            <Text key={rp.professor.id} style={styles.bancaItem}>
              {rp.professor.name}
              {lider && rp.professor.id === lider.id ? ' 👑 Líder' : ''}
            </Text>
          ))
        )}
      </View>
...
const styles = StyleSheet.create({
  ...
  fecharBtn:         { borderWidth: 1.5, borderColor: '#dc2626', borderRadius: 10, padding: 14, alignItems: 'center', marginBottom: 20 },
